...
# This script generates shell scripts that run feature selection hyperparamter tuning on compound structure-target binding datasets.
...
output_folder	<- "data/compound_target_0.25_binary_feature_select_tuning/";
job_name	<- "select_feature_by_cv_target_0.25_binary_tuning";
...
# output file name
all_files_wd <- sapply(all_files, function(af) paste(target_folder, af, "_train.tsv", sep = ""))
writeLines(all_files_wd, paste(output_folder, descriptor_type, "_sample_files.txt", sep = "")) 
...
part[[3]] <- c("0 0.1 classification", "1 0.1 classification");
