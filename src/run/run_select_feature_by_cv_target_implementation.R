...
# This script generates shell scripts that implement feature selection pipeline on compound-target interaction datasets.
...
commands <- mapply(function(af, afi1, afi2){
	...
	# prediction file 
	af_pred <- paste("/home/yunhao1/project/chemical/data/offsides_compounds/", strsplit(afi1, "_")[[1]][[1]], "_combined/offsides_compounds_", afi1, ".tsv", sep = "")	
	# output prefix 
	af_out <- paste("data/compound_target_feature_select_implementation/", afi1, "/", af, sep = "")
...
generate.parallel.bash.files(commands, as.integer(N_cores), paste('select_feature_by_cv_target_implementation', descriptor_type, sep = "_"), "src/run/");
