...
def main(argv):
	## 0. Input arguments:
		...
		# argv 5: input feature type: 'structure' or 'target'
			
	## 1. Read in selected features of models
	feature_df = pd.read_csv(argv[1], sep = '\t', header = 0)
	testing_perf_df = pd.read_csv(argv[2], sep = '\t', header = 0)
	...
	if argv[5] == 'structure':
		...
		target_feature_df = pd.DataFrame({'target': target, 'measure': measure_class, 'structure_features': feature_df.structure_features.values})	
		target_feature_df.index = feature_df.target_measurement.values
		target_feature_df = pd.merge(target_feature_df, testing_perf_df, left_index = True, right_index = True)
		...
		# compare and visualize comparison of testing performance across different model classes 
		perf_compare = ttox_plot.visualize_class_performance_comparison(target_class_feature_df, testing_perf_df.columns[0], argv[4] + '_performance')
		...
		measure_sim_df, measure_sim_pv = ttox_sim.group_pairwise_similarity_by_class(feature_sim_df.feature_jaccard_similarity.values, measure_class)			 
		# compare and visualize the distribution of pairwise similarity scores across different measurement classes 
		measure_compare = ttox_plot.visualize_class_similarity_comparison(measure_sim_df, measure_sim_pv, 'Measurement', argv[4] + '_measure_similarity')	
		...
		class_compare = ttox_plot.visualize_class_similarity_comparison(class_sim_df, class_sim_pv, 'Function class', argv[4] + '_function_similarity')  
