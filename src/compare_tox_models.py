...
def main(argv):
	## 0. Input arguments 
		...
		# argv 2: number of times to repeat the model training process, the average prediction from all models will be used to evaluate model performance  
		# argv 3: output file that contains computed AUC and confidence interval metrics  
		# argv 4: plot file that contains ROC curve of models  
		# argv 5: prefix of plot file contains scatter plots comparing AUROC   
	...
	for oh in optimal_hps:
		...
		oh_perf_dict['structure_model_auc'], oh_perf_dict['structure_model_auc_ci'], oh_perf_dict['structure_model_bootstrap_prop'], structure_fpr, structure_tpr = ttox_learning.compute_classification_model_auc_ci(str_X_train, str_X_test, str_y_train, str_y_test, oh_values[6], n_repeat = int(argv[2]))
		...
		oh_perf_dict['target_all_model_auc'], oh_perf_dict['target_all_model_auc_ci'], oh_perf_dict['target_all_model_bootstrap_prop'], all_tar_fpr, all_tar_tpr = ttox_learning.compute_classification_model_auc_ci(tar_X_train, tar_X_test, tar_y_train, tar_y_test, oh_values[6], n_repeat = int(argv[2]))	
...
#		oh_roc.append((all_tar_fpr, all_tar_tpr, all_tar_label))
		...
		roc_compare = ttox_plot.visualize_model_roc_comparison(oh_roc, ae_name, argv[4] + '_' + str(argv[2]) + '_' + ae_name)	
	
	## 3. Output performance metrics and selected features in data frame format 
	# performance metrics
	perf_df = pd.DataFrame(perf_dict).T
	perf_df.to_csv(argv[3] + '_' + str(argv[2]) + '_testing_performance_summary_auc_ci.tsv', sep = '\t', float_format = '%.5f')
	# all features 
	all_feat_df = pd.DataFrame({'adverse_event': perf_df.index, 'all_features': ae_all_features}) 
	all_feat_df.to_csv(argv[3] + '_all_features.tsv', sep = '\t', index = False, header = True)
	# selected features
	select_feat_df = pd.DataFrame({'adverse_event': perf_df.index, 'select_features': ae_select_features})	
	select_feat_df.to_csv(argv[3] + '_select_features.tsv', sep = '\t', index = False, header = True)
